...
namespace HappyIndexService.Data {
	...
	public class DBCommand : IDisposable {
		...
		#endregion
		#region public bool? GetNullableBit( int column )
		/// <summary>
		/// 
		/// </summary>
		/// <param name="column"></param>
		/// <returns></returns>
		public bool? GetNullableBit( int column ) {
			if( IsDBNull( column ) ) {
				return null;
			}
			return GetBit( column );
		}
		#endregion
		#region public bool? GetNullableBit( string columnName )
		/// <summary>
		/// 
		/// </summary>
		/// <param name="columnName"></param>
		/// <returns></returns>
		public bool? GetNullableBit( string columnName ) {
			return GetNullableBit( GetOrdinal( columnName ) );
		}
		#endregion
		...
		private void PrepareExecution() {
			...
			if( !string.IsNullOrEmpty( commandLog ) ) {
				...
				lock( logFileLock ) {
					try {
						System.IO.File.AppendAllText( commandLog, GetCommandLog() );
					} catch(Exception) {
					}
				}
			}
		...
		public static List<string> GetTableColumnNames( SqlConnection con, string tableName, bool addTableNameToFields ) {
			...
			c.CommandText = string.Format( @"SELECT column_name
FROM information_schema.columns
WHERE table_name = @table_name{0}
ORDER BY ordinal_position", " AND TABLE_SCHEMA = @TABLE_SCHEMA".IfTrue( schema != null ) );
			c.AddWithValue( "@table_name", tableName );
			if( schema != null ) {
				c.AddWithValue( "@TABLE_SCHEMA", schema );
			}
			try {
				while( c.Read() ) {
		...
		#endregion
		public static List<string> GetTableColumnSpecifications( SqlConnection con, string tableName ) {
			List<string> list = new List<string>();
			using( DBCommand c = new DBCommand( con, CommandType.Text ) ) {
				try {
					c.CommandText = string.Format( @"SELECT *
FROM information_schema.columns
WHERE table_name = @table_name
ORDER BY ordinal_position" );
					c.AddWithValue( "@table_name", tableName );
					while( c.Read() ) {
						string colName = c.GetString( "COLUMN_NAME" );
						string colType = c.GetString( "DATA_TYPE" );
						int? colLength = c.GetNullableInt( "CHARACTER_MAXIMUM_LENGTH" );
						if( colLength.HasValue && string.Equals( colType, "ntext", StringComparison.OrdinalIgnoreCase ) ) {
							colLength = null;
						}
						string strColLength = null;
						if( colLength.HasValue ) {
							strColLength = colLength.Value.ToString();
							if( colLength.Value > 8000 ) {
								strColLength = "max";
							}
						}
						bool isNullable = string.Equals( c.GetString( "IS_NULLABLE" ), "yes", StringComparison.OrdinalIgnoreCase );
						list.Add( string.Format( "{0} {1} {2} {3}NULL", colName, colType, "({0})".FillBlanks( strColLength ).IfTrue( strColLength != null ), "NOT ".IfTrue( !isNullable ) ) );
					}
				} catch {
				}
			}
		...
		public List<string> GetAllParamsAndValues() {
			List<string> list = new List<string>();
			foreach( SqlParameter p in cmd.Parameters ) {
				list.Add( string.Format( "{0} = {1} ({2})", p.ParameterName, p.Value, p.DbType ) );
			}
			return list;
		}
	}
